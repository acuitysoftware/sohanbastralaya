				...
				<div class="row">
					<div class="col-xl-9">
						<div class="row gy-2 gx-2 align-items-center justify-content-xl-start justify-content-between">
							<div class="col-auto">
								<div class="mb-3">
									<input class="form-control" wire:model="searchName" type="text" placeholder="Search">
								</div>
							</div>
						</div>
					</div>
					...
					<table class="table table-bordered table-centered w-100 dt-responsive nowrap">
						...
						<tbody>
							...
							<tr>
                                ...
                                @endif
								<td>{{$row->selling_price*$avl_qty}}</td>
                                @if(Auth::user()->type=='A')
								    <td>{{$row->purchase_price*$avl_qty}}</td>
                                @endif
								<td style="white-space: nowrap;"><a href="javascript:void(0);" class="action-icon" wire:click="viewOrder({{$row->id}})"><i class="mdi mdi-eye"></i></a></td>
							</tr>
							@endforeach
                            @else
                            <tr>
